...
def ConvertText(content):
    ...
    #browser.find_element(By.CSS_SELECTOR, 'input[class="select2-search__field"]').send_keys("127\n")
    browser.find_element(By.CSS_SELECTOR, 'a[title="Convert now"]').click()
    wait = WebDriverWait(browser, 100)
    wait.until(expected_conditions.visibility_of_element_located((By.CSS_SELECTOR, 'a[class="btn btn-primary"]')))
    browser.find_element(By.CSS_SELECTOR, 'a[class="btn btn-primary"]').click()
    try:
...
f = open(os.getcwd() + "//NewsPageLink.txt", "r")
Links = f.readlines()
f.close()
...
# Login to TTS website
Login()

# Convert Text to mp3
for i in Queue:
    f = open(os.getcwd() + "//Data//" + date + str(i) + ".txt", "r")
    ConvertText(f.read())
    "en-IN"
    f.close()
