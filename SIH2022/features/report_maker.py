...
def ViewTab(textfile):
    count = 0
    f = open("data\%s"%textfile,"r")
    content = f.read()
    content = content.split("\n")
    sample = content[0]
    global lab
    lab = content[1]
    s = open(r"data/Limits/%s.txt"%sample,"r")
    ...
    return r"data\Reports\Report.docx"
f = open("%s"%"data\Samples\Sample1.txt","r")
content = f.read()
content = content.split("\n")
print(content)
sample = content[0]
lab = content[1]
doc = docx.Document()
createNew("data\Reports\Report.docx")
doc.save(ViewTab(r"\Samples\Sample1.txt"))
docx2pdf.convert(r"data\Reports\Report.docx")
f.close()