...
def show_accounts(ev):
    ...
    for i in list.curselection():
        ...
        for i in output_frame.winfo_children():
            i.destroy()
        Label(output_frame, text="Store Name:").grid(row=0,column=0)
        Label(output_frame, text=store_name).grid(row=1,column=0)
        Label(output_frame, text="Amount:").grid(row=0,column=1)
...
output_frame = Frame(root,highlightbackground="black", highlightthickness=2,padx=10,pady=10 )
